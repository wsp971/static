	    ...
	    function L(a, b, c) {
	        if (b.enablePagination) {
	            var d = '<{0} class="actions {1}"><ul role="menu" aria-label="{2}">{3}</ul></{0}>',
	                e = '<li><a  class="{0}" href="#{1}" role="menuitem">{2}</a></li>',
	                f = "";
	            b.forceMoveForward || (f += e.format("previous","previous", b.labels.previous)), f += e.format("next","next", b.labels.next), b.enableFinishButton && (f += e.format("finish","finish", b.labels.finish)), b.enableCancelButton && (f += e.format("cancel","cancel", b.labels.cancel)), a.append(d.format(b.actionContainerTag, b.clearFixCssClass, b.labels.pagination, f)), D(a, b, c), A(a, b, c)
	        }
	    }
