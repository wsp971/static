...
var pwdMg = {
    ...
    setTitle: function () {
        $(".module h4").html("修改密码");
        $(".module").attr("href", "javascriip:;");
    },
    email: function () {
        $("#send-email").on("click", function () {
            if (pwdMg.validate(true, true) === false) {
                return false;
            }
            email.sendEmail();
        });
    ...
    validate: function (arguments/*, ignore*/) {
        for (var i in validate_rules) {
            if (validate_rules[i].call(this, arguments/*, ignore*/) === false) {
                return false;
            }
        }
    },
    ...
    minitSubmit: function () {
        setInterval(function () {
            if (pwdMg.validate(false/*, false*/) === false) {
    ...
    bindSubmit: function () {
        ...
        $("#btnSubmit").on('click', function () {
            var userId = cookies.get("userId")
            var username = cookies.get("username")
            var data = "username="+username+"&userId=" + userId + "&oldPassword=" +
						$('#iptopdpwd').val().trim() + "&newPassword=" + $('#iptnewpwd').val().trim()
						+ "&reNewPassword=" + $('#iptrenewpwd').val().trim()/* + "&emailVerifyCode=" + $('#iptyzm').val().trim();*/

            if (self.validate(true, false) === false) {
                return;
            }
            loadAjax({
                url: "/user/changePasswordTest",
                data: data,
                type: "post",
                dataType: "json",
                ...
                success: function (result) {
                    if (result) {
                        if (result["rtnCode"] == "000") {
                            layer.alert('密码修改成功，现在去登录？', { closeBtn: 0,
                                yes: function () {
                                    location.href = "../../../page/login/loginPage.html";
                                }
                            });
                        } else if(result["rtnCode"] == "905") {
                            layer.alert('原始密码错误！');
                        }
                    } else {
